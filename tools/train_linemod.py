...
train_cfg['model_name'] = '{}_{}'.format(args.linemod_cls,
...
recorder = Recorder(
    True, os.path.join(cfg.REC_DIR, train_cfg['model_name']),
...
class TransferNetWrapper(nn.Module):
    def __init__(self, net):
        super(TransferNetWrapper, self).__init__()
        self.net = net
    ...
    def forward(self, image, mask, image_render, vertex, vertex_weights):
        ratio = train_cfg['vertex_loss_ratio']
        seg_pred, vertex_pred = self.net(image_render, 'direct')
        loss1, loss_seg, loss_vertex = self.compute_loss(seg_pred, vertex_pred, mask, vertex,
                                   vertex_weights, ratio)
        precision, recall = compute_precision_recall(seg_pred, mask)
        loss1 = torch.mean(loss1)
        
        return seg_pred, vertex_pred, loss_seg, loss_vertex, precision, recall, loss1

def transfer_train(net, optimizer, dataloader, epoch):
    for rec in recs:
        rec.reset()
    data_time.reset()
    batch_time.reset()
    ...
    for idx, data in enumerate(dataloader):
        image, mask, image_render, mask_render, vertex, vertex_weights, pose, _ = [
            d.cuda() for d in data
        ]
        data_time.update(time.time() - end)

        seg_pred, vertex_pred, loss_seg, loss_vertex, precision, recall, loss1 = net(
        ...
        #     rec.update(val)
        loss_seg, loss_vertex, precision, recall, loss1 = [
            torch.mean(val)
            for val in (loss_seg, loss_vertex, precision, recall, loss1)
        ]
        ...
        if idx % train_cfg['img_rec_step'] == 0:
            ...
            recorder.rec_vertex(vertex_pred,
                                ...
                                nrow=4,
                                step=step,
                                name='train/image/ver')

    print('epoch {} training cost {} s'.format(epoch,
                                               time.time() - train_begin))

def transfer_val(net,
        dataloader,
        epoch,
        val_prefix='val',
...
def transfer_train_net():
    ...
    if args.test_model:
        ...
        for epoch in range(begin_epoch, train_cfg['epoch_num']):
            ...
            transfer_train(net, optimizer, train_loader, epoch)
            transfer_val(net, val_loader, epoch, use_motion=motion_model)
            # if args.linemod_cls in cfg.occ_linemod_cls_names:
